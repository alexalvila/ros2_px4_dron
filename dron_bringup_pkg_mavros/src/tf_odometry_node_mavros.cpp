...
public:
  ...
  : Node("tf_odometry_node_mavros")
  {
    ...
    if (publish_odom_msg_) {
      // Reliable para que el display Odometry de RViz conecte sin problemas.
    ...
    // SensorDataQoS para ser compatible con MAVROS si publica odometría como best effort.
    ...
    RCLCPP_INFO(get_logger(), "Subscrito a odometria MAVROS: %s", odom_topic_.c_str());
    RCLCPP_INFO(get_logger(), "Publicando odometria para RViz en: %s", odom_msg_topic_.c_str());
    RCLCPP_INFO(get_logger(), "Publicando TF dinamico: %s -> %s", odom_frame_.c_str(), base_frame_.c_str());
    RCLCPP_INFO(get_logger(), "zero_initial_pose: %s", zero_initial_pose_ ? "true" : "false");
    RCLCPP_INFO(get_logger(), "base_z_offset: %.3f", base_z_offset_);

    if (publish_map_to_odom_) {
      RCLCPP_INFO(get_logger(), "Publicando TF estatico: %s -> %s", map_frame_.c_str(), odom_frame_.c_str());
    }
  }
